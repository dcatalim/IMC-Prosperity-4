...
class Trader:
    ...
    def get_book_stats(self, od: OrderDepth):
        if not od:
            return None
    ...
    def trade_velvet(self, state: TradingState) -> Tuple[List[Order], float]:
        product = VELVET
        v_depth = state.order_depths.get(product)
        ...
        if v_stats:
            ...
            if pos < limit:
                v_orders.append(Order(product, bid_price, min(20, limit - pos)))
            if pos > -limit:
                v_orders.append(Order(product, ask_price, -min(20, pos + limit)))
        
            return v_orders, velvet_fair
