...
# ─── TREND IDENTIFICATION (Based on your provided 3-day PnL analysis) ────────
TREND_UP = {
...
TREND_DOWN = {
    ...
    'SNACKPACK_PISTACHIO', 'SNACKPACK_CHOCOLATE', 'ROBOT_IRONING',
    'ROBOT_VACUUMING', 'MICROCHIP_TRIANGLE', 'MICROCHIP_RECTANGLE',
    'ROBOT_LAUNDRY', 'TRANSLATOR_SPACE_GRAY', 'TRANSLATOR_ASTRO_BLACK',
}
...
class Trader:
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if product in TREND_UP:
                result[product] = self._trend_trade(
                    product, order_depth, position, +POSITION_LIMIT, fair_value, spread
                )
    ...
    def _trend_trade(
        ...
        aggressive_margin = max(1.0, spread * 0.75)
        ...
        if remaining > 0:
            ...
            if remaining > 0:
                ...
                if passive_price < min(order_depth.sell_orders.keys()):
                    orders.append(Order(product, passive_price, remaining))
        else:
            ...
            if qty_to_sell > 0:
                ...
                if passive_price > max(order_depth.buy_orders.keys()):
                    orders.append(Order(product, passive_price, -qty_to_sell))
    ...
    def _market_make(
        ...
        buy_size = min(4, buy_capacity)
        sell_size = min(4, sell_capacity)
