...
for _product, _target in STRUCTURAL_TARGETS.items():
    GROUP_TARGETS.setdefault(group_for(_product), {})[_product] = _target
...
class Trader:
    ...
    def _update_group_gate(self, data: Dict, group: str, paper_pnl: float, gross: int) -> bool:
        ...
        turnup_threshold = max(1500.0, 45.0 * gross)
        momentum_threshold = max(180.0, 5.0 * gross)
    ...
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if target is None:
                target = position if product in STRUCTURAL_TARGETS and position != 0 else 0
