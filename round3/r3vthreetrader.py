...
class Trader:
    ...
    def hydro_orders(
        ...
        short = self.update_ema(cache, "ema_short", self.HYDRO, book["mid"], 0.16)
        long = self.update_ema(cache, "ema_long", self.HYDRO, book["mid"], 0.04)
        mad = self.update_mad(cache, self.HYDRO, book["mid"], long, 0.08)

        fair = (
            10000.0
            + 0.85 * (book["micro"] - book["mid"])
            + 0.20 * (10000.0 - long)
            + 0.18 * (short - long)
        )
        late = self.is_closeout(state)
        edge = 2.0 if abs(position) < 60 else 4.0
        if late:
            edge += 0.5
        ...
        width = self.clamp(1.6 + 0.50 * book["spread"] + 0.35 * mad + (1.0 if late else 0.0), 2.0, 5.5)
        reservation = fair - (0.045 if late else 0.040) * position
        orders.extend(
            self.make_quotes(
                self.HYDRO,
                book,
    ...
    def velvet_orders(
        ...
        short = self.update_ema(cache, "ema_short", self.VELVET, book["mid"], 0.15)
        long = self.update_ema(cache, "ema_long", self.VELVET, book["mid"], 0.05)
        fair = 0.15 * book["micro"] + 0.60 * short + 0.25 * long
        mad = self.update_mad(cache, self.VELVET, book["mid"], fair, 0.08)

        late = self.is_closeout(state)
        take_edge = self.clamp(
            4.0 + 0.75 * book["spread"] + 1.20 * mad + (1.0 if late else 0.0),
            5.5,
            13.0,
        )
        ...
        reservation = fair - (0.090 if late else 0.080) * position
        orders.extend(
            self.make_quotes(
                self.VELVET,
                book,
    ...
    def voucher_orders(
        ...
        spread = max(1.0, book["spread"])
        width = self.clamp(0.55 * spread + 0.75 + (0.25 if late else 0.0), 1.0, 8.0)
        ...
        orders, position = self.take_orders(
            ...
            fair - take_edge,
            fair + take_edge,
            max_clip=min(size, 1 if late else 2),
        )
        ...
        reservation = fair - 0.045 * position
        orders.extend(
            self.make_quotes(
                symbol,
                book,
    ...
    def run(self, state: TradingState):
        ...
        if velvet_fair is not None:
            ...
            sigma = 0.20 * raw_sigma + 0.80 * float(cache.get("sigma", raw_sigma))
            cache["sigma"] = sigma
            late = self.is_closeout(state)

            if sigma_rmse <= 9.0 and not late:
                for symbol in self.VOUCHERS:
                    orders = self.voucher_orders(symbol, state, velvet_fair, sigma)
                    if orders:
                        result[symbol] = orders
            elif sigma_rmse <= 7.5 and late:
                for symbol in self.VOUCHERS:
                    orders = self.voucher_orders(symbol, state, velvet_fair, sigma)
                    if orders:
                        result[symbol] = orders
            else:
                LOGGER.print("Skipping vouchers, surface fit too noisy or closeout:", round(sigma_rmse, 3), "late=" , late)
