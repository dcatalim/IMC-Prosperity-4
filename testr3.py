...
class Trader:
    ...
    def liquidation_orders(self, state: TradingState) -> dict[Symbol, list[Order]]:
        ...
        for symbol, position in state.position.items():
            ...
            if book is None:
                continue
            if position > 0:
                quantity = min(position, self.limits.get(symbol, position))
                result.setdefault(symbol, []).append(
                    Order(symbol, int(book["best_bid"]), -quantity)
                )
            else:
                quantity = min(-position, self.limits.get(symbol, -position))
                result.setdefault(symbol, []).append(
                    Order(symbol, int(book["best_ask"]), quantity)
                )
        return result
    ...
    def run(self, state: TradingState):
        ...
        if state.timestamp >= 91000:
            result = self.liquidation_orders(state)
