...
class Trader:
    ...
    def _get_sentiment_adjustment(self, metrics: TraderMetrics, product: str) -> float:
        """
        ...
        Returns adjustment as % of mid price (e.g., 0.01 = +1%)
        """
        ...
        # Convert to price adjustment (-2% to +2%)
        adjustment = (sentiment - 0.5) * 0.04
        return adjustment
    ...
    def trade_hydro(self, state: TradingState, mem: Dict) -> List[Order]:
        ...
        # Apply sentiment adjustment based on buyer/seller activity
        metrics = self._update_trader_metrics(state, mem)
        sentiment_adj = self._get_sentiment_adjustment(metrics, product)
    ...
    def trade_vouchers_simple(self, state: TradingState, mem: Dict, S: float) -> List[Order]:
        ...
        for sym, K in VOUCHERS.items():
            ...
            # Apply sentiment adjustment based on buyer/seller activity
            sentiment_adj = self._get_sentiment_adjustment(metrics, sym)
            fair = fair * (1.0 + sentiment_adj)
