...
class Trader:
    ...
    def hydro_orders(
        ...
        width = self.clamp(1.6 + 0.50 * book["spread"] + 0.35 * mad, 2.0, 4.5)
        reservation = fair - 0.040 * position
        orders.extend(
            self.make_quotes(
                self.HYDRO,
                book,
    ...
    def velvet_orders(
        ...
        take_edge = self.clamp(4.0 + 0.75 * book["spread"] + 1.20 * mad, 5.5, 12.0)
        ...
        width = self.clamp(2.0 + 0.65 * book["spread"] + 0.80 * mad, 3.0, 8.0)
        reservation = fair - 0.080 * position
        orders.extend(
            self.make_quotes(
                self.VELVET,
                book,
    ...
    def voucher_orders(
        ...
        spread = max(1.0, book["spread"])
        width = self.clamp(0.55 * spread + 0.75, 1.0, 8.0)
        take_edge = width + 2.0
    ...
    def run(self, state: TradingState):
        ...
        if velvet_fair is not None:
            ...
            if sigma_rmse <= 9.0:
                ...
                LOGGER.print("Skipping vouchers, surface fit too noisy:", round(sigma_rmse, 3))
