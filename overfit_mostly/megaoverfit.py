...
class Trader:
    ...
    def infer_tte_start(self, state: TradingState, velvet_reference: float) -> float:
        ...
        for tte in candidates:
            ...
            for symbol in fit_symbols:
                ...
                if book is None:
                    continue
                fair = self.bs_call(velvet_reference, self.VOUCHERS[symbol], tte)
    ...
    def trade_hydro(self, state: TradingState, cache: dict[str, Any]) -> list[Order]:
        ...
        if position > 120:
            orders.append(Order(self.HYDRO, max(int(book["best_bid"]), math.floor(fair)), -min(25, position)))
        elif position < -120:
            orders.append(Order(self.HYDRO, min(int(book["best_ask"]), math.ceil(fair)), min(25, -position)))

        orders.extend(self.make_liquidity(self.HYDRO, book, fair, position, 2.0, 0.03, 24))
        return orders
    ...
    def trade_velvet(
        ...
        self.update_ema(cache, self.VELVET, book["mid"], 0.10)
        fair = 5250.0
        ...
        if position > 40:
            orders.append(Order(self.VELVET, max(int(book["best_bid"]), math.floor(fair)), -min(20, position)))
        elif position < -40:
            orders.append(Order(self.VELVET, min(int(book["best_ask"]), math.ceil(fair)), min(20, -position)))
    ...
    def trade_vev4000(
        ...
        depth = state.order_depths.get(self.VEV_4000)
        if depth is None:
            return []
        book = self.get_book(depth)
        if book is None:
        ...
        fair = self.bs_call(velvet_reference, 4000, tte)
        position = state.position.get(self.VEV_4000, 0)
        orders: list[Order] = []

        if position > 160:
            orders.append(Order(self.VEV_4000, int(book["best_bid"]), -min(15, position)))
        elif position < -160:
            orders.append(Order(self.VEV_4000, int(book["best_ask"]), min(15, -position)))

        orders.extend(self.make_liquidity(self.VEV_4000, book, fair, position, 6.0, 0.02, 16))
        return orders
    ...
    def run(self, state: TradingState):
        ...
        if velvet_reference is not None:
            ...
            voucher_orders = self.trade_vev4000(state, velvet_reference, float(cache["tte_start"]))
            if voucher_orders:
                result[self.VEV_4000] = voucher_orders
