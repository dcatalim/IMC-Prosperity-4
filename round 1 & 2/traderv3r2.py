    ...
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ...
                my_bid = math.floor(fair_value) - MM_SPREAD
                my_ask = math.ceil(fair_value) + MM_SPREAD

                if len(order_depth.sell_orders) != 0:
                    for ask in sorted(order_depth.sell_orders.keys()):
                        if ask <= fair_value - TAKE_EDGE or (
                            state.timestamp < 1000 and current_pos < TARGET_HOLD and ask <= fair_value
                        ):
                            ask_amount = order_depth.sell_orders[ask]
                            if state.timestamp < 1000 and ask > fair_value - TAKE_EDGE:
